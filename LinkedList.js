...
class LinkedList {
  ...
  traverse(callback) {
    ...
    while (currentNode) {
      // Call the callback with the current node and capture the result
      const result = callback(currentNode);

      // Exit the loop and return the result if the callback returns a value
      if (result !== undefined) {
        return result;
      }
  ...
  // Returns true if the key is in the list and otherwise returns false.
  contains(key) {
    const contains = (node) => {
      if (node.key == key) {
        return true; // Stop traversal
  ...
  toString() {
    ...
    const toString = (node) => {
      ...
      resultString += `( ${node.key} )${node.next ? ' => ' : ''}`;
    };
  ...
  // Inserts a new node with the provided key at the given index
  insertAt(index, key, value = null) {
    const node = new Node(key, value);
